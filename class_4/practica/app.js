...
function enviarConfirmacionReserva(nombreCliente) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            const correoEnviado = Math.round(Math.random());
            if(correoEnviado===1){
                resolve(`Se ha realizado la reserva a nombre de ${nombreCliente}, y se ha enviado la confirmación al correo.`)
            }else{
                reject(`Se ha realizado la reserva a nombre de ${nombreCliente}, sin embargo no se a logrado enviar la confirmación al correo.`)
            }
...
async function hacerReserva(nombreCliente, mesasSolicitadas) {
    try {
        ...
        if(disponibilidad){
            const msn = await enviarConfirmacionReserva(nombreCliente);
            console.log(msn);
